...
class RenovationAnalyzer:
    ...
    def create_analysis_prompt(self, category: str, category_items: List[Dict], photo_path: str) -> str:
...
"""
    ...
    def analyze_photo_with_gemini(self, photo_path: Path, category: str, category_items: List[Dict]) -> Dict:
        ...
        try:
            ...
            # Create prompt
            prompt = self.create_analysis_prompt(category, category_items, str(photo_path))
                ...
                if json_match:
                    try:
                        ...
                        if json_match:
                            ...
                            raise json.JSONDecodeError("No JSON structure found", response_text, 0)
                else:
            ...
            logger.warning(f"Failed to parse JSON for {photo_path}. Raw response: {response_text[:500]}...")
            return {
                ...
                "parsed_analysis": self.extract_key_info_from_text(response_text, category)
            }
    ...
    def extract_key_info_from_text(self, text: str, category: str) -> Dict:
        ...
        condition_match = re.search(r'condition["\s:]+(["\']?)(excellent|good|fair|poor|critical)\1', text, re.IGNORECASE)
        if condition_match:
            extracted["condition"] = condition_match.group(2).lower()
        
        # Try to extract years since renovation
        years_since_match = re.search(r'years[_\s]+since[_\s]+renovation["\s:]+(["\']?)(\d+)\1', text, re.IGNORECASE)
        if years_since_match:
            extracted["years_since_renovation"] = int(years_since_match.group(2))
        
        # Try to extract years until renovation
        years_until_match = re.search(r'years[_\s]+until[_\s]+renovation["\s:]+(["\']?)(\d+)\1', text, re.IGNORECASE)
        if years_until_match:
            extracted["years_until_renovation"] = int(years_until_match.group(2))
        
        # Try to extract costs
        immediate_cost_match = re.search(r'immediate[_\s]+.*?cost["\s:]+(["\']?)(\d+(?:,\d{3})*(?:\.\d+)?)\1', text, re.IGNORECASE)
        ...
        future_cost_match = re.search(r'future[_\s]+.*?cost["\s:]+(["\']?)(\d+(?:,\d{3})*(?:\.\d+)?)\1', text, re.IGNORECASE)
        ...
        urgency_match = re.search(r'urgency[_\s]*level["\s:]+(["\']?)(immediate|urgent|moderate|low)\1', text, re.IGNORECASE)
        if urgency_match:
    ...
    def analyze_category(self, folder_name: str) -> List[Dict]:
        ...
        for i, photo_path in enumerate(photos):
            ...
            result = self.analyze_photo_with_gemini(photo_path, csv_category, category_items)
    ...
    def save_results(self, results: Dict[str, List[Dict]], output_file: str = "renovation_analysis_results.json"):
    ...
    def generate_summary_report(self, results: Dict[str, List[Dict]]) -> str:
        ...
        for folder_name, analyses in results.items():
            ...
            for analysis in analyses:
                if 'error' in analysis:
                    report.append(f"- **Error analyzing {Path(analysis['photo_path']).name}**: {analysis['error']}")
                    
                    # If we have parsed analysis despite the error, include it
                    if 'parsed_analysis' in analysis:
                        parsed = analysis['parsed_analysis']
                        report.append(f"  - **Extracted condition**: {parsed.get('condition', 'unknown')}")
                        report.append(f"  - **Extracted years since renovation**: {parsed.get('years_since_renovation', 'unknown')}")
                        report.append(f"  - **Extracted immediate cost**: {parsed.get('immediate_cost', 0)} CHF")
                ...
                if 'photo_analysis' in analysis:
                    condition = analysis['photo_analysis'].get('overall_condition', 'unknown')
                    report.append(f"- **Condition**: {condition}")
                
                if 'age_assessment' in analysis:
                    years_since = analysis['age_assessment'].get('estimated_years_since_renovation', 'unknown')
                    confidence = analysis['age_assessment'].get('confidence_level', 'unknown')
                    report.append(f"- **Years since renovation**: {years_since} (confidence: {confidence})")
                
                if 'renovation_prediction' in analysis:
                    years_until = analysis['renovation_prediction'].get('years_until_renovation_needed', 'unknown')
                    urgency = analysis['renovation_prediction'].get('urgency_level', 'unknown')
                    report.append(f"- **Years until renovation**: {years_until} (urgency: {urgency})")
                    
                    if urgency in ['immediate', 'urgent']:
                        urgent_items.append(f"{folder_name}/{photo_name}")
                
                if 'cost_analysis' in analysis:
                    immediate = analysis['cost_analysis'].get('immediate_repairs', {}).get('estimated_cost_chf', 0)
                    future = analysis['cost_analysis'].get('future_renovation', {}).get('estimated_cost_chf', 0)
        ...
        report.append(f"- **Total immediate repair costs**: {total_immediate_cost:,.0f} CHF")
        report.append(f"- **Total future renovation costs**: {total_future_cost:,.0f} CHF")
        report.append(f"- **Total estimated costs**: {(total_immediate_cost + total_future_cost):,.0f} CHF")
